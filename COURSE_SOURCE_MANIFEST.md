# Course Source Manifest

Canonical source repository: `APCS`

## Mapped Catalog Courses

- `java-level-1`: Java Level 1
- `java-without-graphics`: Java without Graphics
- `java-with-graphics`: Java with Graphics
- `ap-computer-science-a`: AP Computer Science A

## Verification Gate

- Run `./verify-course-source.sh` from this repository root before treating the source pack as ready.
- The verification gate checks for this manifest, the source backlog ledger, source-like files, removed Replit metadata, and any
  repo-specific readiness files.
- Project-specific unit tests or build commands should still be run inside individual project folders when a project includes its own test
  harness.

## Active Catalog Targets

| Folder                                         |
|------------------------------------------------|
| `APCS-A-2020-Practice-Exam`                    |
| `APCS-Check-in-1`                              |
| `APCS-Check-in-1-Additional-Practice-Project`  |
| `APCS-Check-in-2`                              |
| `APCS-Check-in-2-Additional-Practice-Project`  |
| `APCS-Check-in-3`                              |
| `APCS-Check-in-3-Additional-Practice-Project`  |
| `APCS-Check-in-4`                              |
| `APCS-Check-in-4-Additional-Practice-Project`  |
| `APCS1-Casting-Reference`                      |
| `APCS1-Hospital-Survey`                        |
| `APCS1-Mad-Libs`                               |
| `APCS1-Rounding-It-Off`                        |
| `APCS1-Strings-and-Printing-Reference`         |
| `APCS1-Variables-Reference`                    |
| `APCS10-Array-Reference`                       |
| `APCS10-Matrix-Arithmetic`                     |
| `APCS10-Practice-With-Arrays`                  |
| `APCS10-Practice-With-Two-Dimensional-Arrays`  |
| `APCS10-Random-Mad-Lib`                        |
| `APCS11-Array-List-Reference`                  |
| `APCS11-Card-and-Deck-Classes`                 |
| `APCS11-Card-Shuffler`                         |
| `APCS11-Fortune-Teller-Class`                  |
| `APCS11-Free-Response-Practice`                |
| `APCS11-Multiple-Choice-Reference`             |
| `APCS11-Practice-With-Array-Lists`             |
| `APCS12-Deck-Class`                            |
| `APCS12-Minimum-and-Maximum`                   |
| `APCS12-Suit-Deck-and-Hand`                    |
| `APCS13-Linear-Search-Implementation`          |
| `APCS13-Runtime-Analysis`                      |
| `APCS14-Insertion-Sort`                        |
| `APCS14-Selection-Sort`                        |
| `APCS15-Blob-Erase`                            |
| `APCS15-Recursion-Practice`                    |
| `APCS15-Recursion-Reference`                   |
| `APCS15-Stack-Overflow-Reference`              |
| `APCS15-Tracing-Recursion`                     |
| `APCS16-Binary-Search`                         |
| `APCS16-Merge-Sort`                            |
| `APCS17-Decode`                                |
| `APCS17-Elevens`                               |
| `APCS17-Spaceships`                            |
| `APCS2-Quotient-and-Remainder`                 |
| `APCS2-Too-Chicken-To-Cross-The-Road`          |
| `APCS2-Two-In-One`                             |
| `APCS2-Verifying-Expressions`                  |
| `APCS3-Conditionals-Reference`                 |
| `APCS3-Elevator-Warning`                       |
| `APCS3-Math-Fun`                               |
| `APCS3-Math-Package-Reference`                 |
| `APCS3-Random-Numbers-Reference`               |
| `APCS3-Too-Chicken-Take-Two`                   |
| `APCS4-For-Loop-Practice`                      |
| `APCS4-Loops-Reference`                        |
| `APCS4-Translating-Loops`                      |
| `APCS4-While-Loop-and-Nested-Loop-Practice`    |
| `APCS4-While-Loops-and-Nested-Loops-Reference` |
| `APCS5-Class-Example`                          |
| `APCS5-Free-Response-Practice`                 |
| `APCS5-Practice-with-Methods`                  |
| `APCS5-Spreadsheet-Width`                      |
| `APCS5-Store-Class`                            |
| `APCS5-Student-Class`                          |
| `APCS6-Bank-Account-Class`                     |
| `APCS6-Farm-Class`                             |
| `APCS6-Free-Response-Practice`                 |
| `APCS6-Getters-and-Setters-Reference`          |
| `APCS6-This-Reference`                         |
| `APCS6-Vending-Machine-Class`                  |
| `APCS7-Books-and-Picture-Books`                |
| `APCS7-Dogs`                                   |
| `APCS7-Multiple-Choice-Reference`              |
| `APCS7-Pets`                                   |
| `APCS8-Many-Shapes`                            |
| `APCS8-Multiple-Choice-Reference`              |
| `APCS8-Polymorphism-Reference`                 |
| `APCS9-Robust-Input-Practice`                  |
| `APCS9-Specification`                          |

## Source Inventory

- Active project folders: 80
- Active linked folders: 80
- Archived inactive/support folders: 26
- Wrapper project folders: 80
- Placeholder role folders awaiting a distinct counterpart: 70
- Complete starter/solution pairs with distinct migrated content: 10
- Active source-like files excluding archive: 334

Notes: active source-like files exclude `_archived-unlinked/`. Placeholder role folders are structural markers only; they do not contain
assignment source yet.
